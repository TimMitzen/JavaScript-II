...
// Problem 1 Need to list all emails and first names
let firstNameEmail = [];
runners.forEach(runner => firstNameEmail.push(`${runner.first_name}, ${runner.email}`));
console.log(firstNameEmail);

// Problem 2 Average of all donations per person
let averageDonations =[];
averageDonations = runners.reduce((avg,runner) => avg + runner.donation,0) / runners.length;
console.log(averageDonations);
// Problem 3 Company names sorted by alphabetical order
let companyList = [];

companyList = runners.map(company => company.company_name)
